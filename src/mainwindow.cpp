    ...
    m_poll(false)
{
    ...
    connect( m_portTimer, SIGNAL(timeout()), this, SLOT(resetStatus()));
    m_portTimer->setSingleShot(true);
...
void MainWindow::busMonitorRawData( uint8_t * data, uint8_t dataLen, bool addNewline, uint8_t direction )
{
    ...
    if( dataLen > 0 )
	{
        QByteArray dataByte = NULL;
        QString dataString = NULL;
		...
		{
            dataString += QString::asprintf( "%.2x ", data[i] );
